...
with tf.Session(config=tf.ConfigProto(inter_op_parallelism_threads=nthreads_inter,
    ...
    for num_batch, batch in enumerate(batches):
        ...
        # Run the session
        _, logits, out_loss, computed_perplexity, train_allState, train_lastState = sess.run([optimizer, softmax_output, loss, perplexity, allState, lastState],
                                                            {x: batch_input,
                                                             label: batch_target,
        ...
        if num_batch % print_every == 0:
            ...
            #greedy predict. we reset the state
            gen_lastState = getDefaultState(1, hidden_size)
            gen_in = np.expand_dims(np.array(['<bos>', 'indeed', 'im', 'ok']),0)
            gen_in = word_to_index_transform(word_to_index, gen_in)
            #update the state with prefix
            for i in range(len(gen_in)): 
                goin = np.expand_dims(gen_in[:,i],1)
                gen_pred_logits = sess.run(gen_pred, {gen_input: goin})
            #predict
            
#            for i in range(30):   
#                goin = np.expand_dims(gen_in[:,i],1)
#                gen_out, gen_pred, gen_lastState = getOneStep(gen_pred, gen_lastState, down_project, batch_size,hidden_size)
#                print(gen_pred)
